Ext.define('Testpro2.testpro2.web.com.controller.appinsight.health.TestTwoMainController', {
     extend: 'Testpro2.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.TestTwoMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#TestTwoTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#TestTwoForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#TestTwoGrid');
               var tree = this.view.down('#TestTwoTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#TestTwoForm');
          var grid = this.view.down('#TestTwoGrid');
          var tree = this.view.down('#TestTwoTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#TestTwoGrid');
               ...
               var formPanel = this.getView().up().down('#TestTwoForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#TestTwoTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#TestTwoForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#TestTwoGrid').store;
          gridStore.removeAll();
