Ext.define('Newpro.newpro.web.com.controller.appinsight.health.TtwoMainController', {
     extend: 'Newpro.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.TtwoMainController',
     init: function() {
          var form = this.view.down('#TtwoForm');
     ...
     tonenmLoad: function() {
          ...
          Ext.Ajax.request({
               url: 'secure/Tone/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#tonenm');
                    scope.sender.controller.assignAllComboData(scope.sender, 'tonenm', response.responseText);
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#TtwoTree');
               ...
               var compRef = this.view.down('#TtwoTree');
     ...
     resetForm: function(but) {
          ...
          form.down('#saveFormButton').setText('Save');
          form.reset();
          var grid = this.view.down('#TtwoGrid');
          var tree = this.view.down('#TtwoTree');
          tree.setSelection();
          grid.setSelection();
     },
     hideDataBrowser: function() {
          var form = this.view.down('#Ttwo');
          var grid = this.view.down('#TtwoGrid');
          var tree = this.view.down('#TtwoTreeContainer');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#TtwoForm');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, opts) {
                    ...
                    var compRef = currentView.down('#TtwoTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          ...
          if (record.data.leaf) {
               var Ttwo = this.view.down('#TtwoForm');
               Ttwo.reset();
               var gridPanel = this.view.down('#TtwoGrid');
          ...
          var formPanel = this.getView().up().down('#TtwoForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#TtwoTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Ttwo').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#TtwoGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
