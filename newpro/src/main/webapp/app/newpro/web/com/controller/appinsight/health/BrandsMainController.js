Ext.define('Newpro.newpro.web.com.controller.appinsight.health.BrandsMainController', {
     extend: 'Newpro.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.BrandsMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#BrandsTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#BrandsForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#BrandsGrid');
               var tree = this.view.down('#BrandsTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#BrandsForm');
          var grid = this.view.down('#BrandsGrid');
          var tree = this.view.down('#BrandsTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#BrandsGrid');
               ...
               var formPanel = this.getView().up().down('#BrandsForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#BrandsTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#BrandsForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#BrandsGrid').store;
          gridStore.removeAll();
