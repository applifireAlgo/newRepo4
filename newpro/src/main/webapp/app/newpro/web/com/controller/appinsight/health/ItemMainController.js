Ext.define('Newpro.newpro.web.com.controller.appinsight.health.ItemMainController', {
     ...
     brandnmLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/Brands/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#brandnm');
                    scope.sender.controller.assignAllComboData(scope.sender, 'brandnm', response.responseText);
     ...
     onBrandnmChange: function(me, newValue, oldValue, eOpts) {
          this.onBrandnmChangeCategorynm(me, newValue, oldValue, eOpts);
     },
     onBrandnmChangeCategorynm: function(me, newValue, oldValue, eOpts) {
          var scope = this.getView();
          Ext.Ajax.request({
               async: false,
               url: 'secure/Category/findByBrandnm',
               method: 'POST',
               jsonData: {
                    findKey: newValue
               },
               ...
               element: me,
               success: function(response, scope) {
                    var categorynmCombo = scope.element.up('form').down('#categorynm');
                    scope.sender.controller.assignComboData(categorynmCombo, response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, categorynmCombo);
               },
               failure: function(response, scope) {
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var compRef = this.view.down('#ItemTree');
