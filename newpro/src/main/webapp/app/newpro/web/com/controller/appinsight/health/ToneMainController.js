Ext.define('Newpro.newpro.web.com.controller.appinsight.health.ToneMainController', {
     extend: 'Newpro.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.ToneMainController',
     init: function() {
          var form = this.view.down('#ToneForm');
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#ToneTree');
               ...
               var compRef = this.view.down('#ToneTree');
     ...
     resetForm: function(but) {
          ...
          form.down('#saveFormButton').setText('Save');
          form.reset();
          var grid = this.view.down('#ToneGrid');
          var tree = this.view.down('#ToneTree');
          tree.setSelection();
          grid.setSelection();
     },
     hideDataBrowser: function() {
          var form = this.view.down('#Tone');
          var grid = this.view.down('#ToneGrid');
          var tree = this.view.down('#ToneTreeContainer');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#ToneForm');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, opts) {
                    ...
                    var compRef = currentView.down('#ToneTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          ...
          if (record.data.leaf) {
               var Tone = this.view.down('#ToneForm');
               Tone.reset();
               var gridPanel = this.view.down('#ToneGrid');
          ...
          var formPanel = this.getView().up().down('#ToneForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#ToneTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Tone').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#ToneGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
