Ext.define('Newpro.newpro.web.com.controller.appinsight.health.TestOneMainController', {
     extend: 'Newpro.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.TestOneMainController',
     init: function() {
          var form = this.view.down('#TestOneForm');
     ...
     aaaLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               success: function(response, scope) {
                    var compRef = scope.sender.down('#aaa');
                    scope.sender.controller.assignAllComboData(scope.sender, 'aaa', response.responseText);
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#TestOneTree');
               ...
               var compRef = this.view.down('#TestOneTree');
     ...
     resetForm: function(but) {
          ...
          form.down('#saveFormButton').setText('Save');
          form.reset();
          var grid = this.view.down('#TestOneGrid');
          var tree = this.view.down('#TestOneTree');
          tree.setSelection();
          grid.setSelection();
     },
     hideDataBrowser: function() {
          var form = this.view.down('#TestOne');
          var grid = this.view.down('#TestOneGrid');
          var tree = this.view.down('#TestOneTreeContainer');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#TestOneForm');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, opts) {
                    ...
                    var compRef = currentView.down('#TestOneTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          ...
          if (record.data.leaf) {
               var TestOne = this.view.down('#TestOneForm');
               TestOne.reset();
               var gridPanel = this.view.down('#TestOneGrid');
          ...
          var formPanel = this.getView().up().down('#TestOneForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#TestOneTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#TestOne').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#TestOneGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
