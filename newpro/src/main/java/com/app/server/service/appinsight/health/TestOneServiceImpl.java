...
public class TestOneServiceImpl extends TestOneService {
    ...
    @RequestMapping(value = "/findByAaa", method = RequestMethod.POST)
    @Override
    public HttpEntity<ResponseBean> findByAaa(@RequestBody FindByBean findByBean) throws Exception {
        org.springframework.http.HttpStatus httpStatus = org.springframework.http.HttpStatus.OK;
        List<com.app.shared.appinsight.health.TestOne> lsttestone = testOnerepo.findByAaa((java.lang.String) findByBean.getFindKey());
        AppAlarm appAlarm = Log.getAlarm("AISHI124100200");
        com.athena.server.pluggable.utils.bean.ResponseBean responseBean = new ResponseBean(appAlarm);
        responseBean.add("message", String.format(appAlarm.getMessage(), "TestOne"));
        responseBean.add("data", lsttestone);
        Log.out.println("AISHI124100200", runtimeLogInfoHelper.getRequestHeaderBean(), "TestOneServiceImpl", "save", "TestOne");
        return new org.springframework.http.ResponseEntity<ResponseBean>(responseBean, HttpStatus.valueOf(appAlarm.getAlarmStatus()));
    }
    ...
    @RequestMapping(value = "/TOneNamtiveQ/{ttid}", method = RequestMethod.POST)
    @Override
    public HttpEntity<ResponseBean> TOneNamtiveQ(@PathVariable String ttid) throws Exception {
        org.springframework.http.HttpStatus httpStatus = org.springframework.http.HttpStatus.OK;
        java.util.List<com.app.shared.appinsight.health.TestOne> lsttestone = testOnerepo.TOneNamtiveQ(ttid);
        AppAlarm appAlarm = Log.getAlarm("AISHI124100200");
        com.athena.server.pluggable.utils.bean.ResponseBean responseBean = new ResponseBean(appAlarm);
        responseBean.add("message", String.format(appAlarm.getMessage(), "TestOne"));
        responseBean.add("data", lsttestone);
