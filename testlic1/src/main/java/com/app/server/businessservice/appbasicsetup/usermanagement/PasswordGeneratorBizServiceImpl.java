...
import com.app.config.mail.EMailSender;
...
import com.spartan.server.password.PasswordGenerator;
import com.spartan.server.password.PasswordLogic;
...
public class PasswordGeneratorBizServiceImpl implements PasswordGeneratorBizService {
	...
	public HttpEntity<ResponseBean> forgetPassword(String data) throws Exception {
		...
		try {
			...
			List<UserAuthentication> lstEntity = authenticateRepo.fetchLoginInfo(loginId);
			if (lstEntity.isEmpty()) {
				...
				if (isValid) {
					...
					UserDataInterface userData = userDataRepo.findByUserId(lstEntity.get(0).getUserId());
	...
	public HttpEntity<ResponseBean> findSecurityQuestions(String loginId) throws Exception {
		...
		try {
			...
			boolean success = true;
			String message = "";
			List<UserAuthentication> entity = authenticateRepo.fetchLoginInfo(loginId);
			if (entity.isEmpty()) {
				success = false;
				message = "Incorrect Username";
				...
				List<PassRecoveryInterface> passRecovery = userManagementRepo.findByUserId(entity.get(0).getUserId());
