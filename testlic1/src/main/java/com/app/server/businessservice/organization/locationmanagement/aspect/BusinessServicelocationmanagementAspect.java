package com.app.server.businessservice.organization.locationmanagement.aspect;
...
public class BusinessServicelocationmanagementAspect extends BusinessAspect {
    ...
    @Pointcut("execution(* com.app.server.businessservice.organization.locationmanagement..*(..))")
    ...
    public Object aroundfindAll(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        try {
            ...
            Log.out.println("ORGLM227100100", runtimeLogInfoHelper.getRequestHeaderBean(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName());
        } catch (Exception e) {
            methodCallDetails.setPostCallDetails(HealthConstants.METHOD_EXCEPTION, HealthConstants.DEFAULT_EXCEPTION_ID);
            BusinessException businessException = new BusinessException();
            businessException.getException("ORG", "LM", e, runtimeLogInfoHelper.getRequestHeaderBean(), joinPoint);
        } finally {
