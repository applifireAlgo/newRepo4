...
@RestController
@SourceCodeAuthorClass(createdBy = "shweta.zagade1209@gmail.com", updatedBy = "", versionNumber = "1", comments = "Service for Login Transaction table", complexity = Complexity.MEDIUM)
...
public class LoginServiceImpl extends LoginService {
    ...
    public HttpEntity<ResponseBean> findByUserId(@RequestBody FindByBean findByBean) throws Exception {
        ...
        responseBean.add("message", String.format(appAlarm.getMessage(), "Login"));
        responseBean.add("data", lstlogin);
        Log.out.println("ABSUM224990200", runtimeLogInfoHelper.getRequestHeaderBean(), "LoginServiceImpl", "save", "Login");
        return new org.springframework.http.ResponseEntity<ResponseBean>(responseBean, HttpStatus.valueOf(appAlarm.getAlarmStatus()));
    }
