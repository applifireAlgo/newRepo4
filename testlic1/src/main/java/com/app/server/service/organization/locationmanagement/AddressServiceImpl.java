...
@RestController
@SourceCodeAuthorClass(createdBy = "shweta.zagade1209@gmail.com", updatedBy = "", versionNumber = "1", comments = "Service for Address Transaction table", complexity = Complexity.MEDIUM)
...
public class AddressServiceImpl extends AddressService {
    ...
    public HttpEntity<ResponseBean> findByAddressTypeId(@RequestBody FindByBean findByBean) throws Exception {
        ...
        responseBean.add("message", String.format(appAlarm.getMessage(), "Address"));
        responseBean.add("data", lstaddress);
        Log.out.println("ORGLM224990200", runtimeLogInfoHelper.getRequestHeaderBean(), "AddressServiceImpl", "save", "Address");
        return new org.springframework.http.ResponseEntity<ResponseBean>(responseBean, HttpStatus.valueOf(appAlarm.getAlarmStatus()));
    }
