Ext.define('Testlic.testlic.web.com.controller.testbc.testing.NameTestMainController', {
     extend: 'Testlic.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.NameTestMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#NameTestTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#NameTestForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#NameTestGrid');
               var tree = this.view.down('#NameTestTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#NameTestForm');
          var grid = this.view.down('#NameTestGrid');
          var tree = this.view.down('#NameTestTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#NameTestGrid');
               ...
               var formPanel = this.getView().up().down('#NameTestForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#NameTestTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#NameTestForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#NameTestGrid').store;
          gridStore.removeAll();
