Ext.define('Testlic.testlic.web.com.controller.appinsight.health.EmpMainController', {
     extend: 'Testlic.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.EmpMainController',
     init: function() {
          var form = this.view.down('#EmpForm');
          form.reset();
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#EmpTree');
               ...
               var compRef = this.view.down('#EmpTree');
     ...
     resetForm: function(but) {
          ...
          form.down('#saveFormButton').setText('Save');
          form.reset();
          var grid = this.view.down('#EmpGrid');
          var tree = this.view.down('#EmpTree');
          tree.setSelection();
          grid.setSelection();
     },
     hideDataBrowser: function() {
          var form = this.view.down('#Emp');
          var grid = this.view.down('#EmpGrid');
          var tree = this.view.down('#EmpTreeContainer');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#EmpForm');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, opts) {
                    ...
                    var compRef = currentView.down('#EmpTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          ...
          if (record.data.leaf) {
               var Emp = this.view.down('#EmpForm');
               Emp.reset();
               var gridPanel = this.view.down('#EmpGrid');
          ...
          var formPanel = this.getView().up().down('#EmpForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#EmpTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Emp').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#EmpGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
