Ext.define('Testaone.testaone.web.com.controller.testbc.testingdomain.StudMainController', {
     extend: 'Testaone.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.StudMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#StudTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#StudForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#StudGrid');
               var tree = this.view.down('#StudTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#StudForm');
          var grid = this.view.down('#StudGrid');
          var tree = this.view.down('#StudTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#StudGrid');
               ...
               var formPanel = this.getView().up().down('#StudForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#StudTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#StudForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#StudGrid').store;
          gridStore.removeAll();
